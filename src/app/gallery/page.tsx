...
export default function GalleryPage() {
  ...
  useEffect(() => {
    fetch('/api/photos')
      .then(res => res.json())
      .then((data: unknown) => {
        setPhotos(Array.isArray(data) ? data as Photo[] : []);
      })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  // Fallback: if a photo request hangs, advance after 6s
  useEffect(() => {
    if (loadedIndex >= photos.length) return;
    const timer = setTimeout(() => setLoadedIndex(i => i + 1), 6000);
  ...
  return (
    <div className="min-h-screen bg-husky-black text-white">
      <div className="max-w-6xl mx-auto px-4 py-20">
        ...
        <p className="text-zinc-400 mb-12">All memories from the Class of 2006</p>
        ...
        {loading ? (
          ...
          <>
            <p className="text-zinc-500 text-sm mb-6">{photos.length} photos</p>
      ...
      {lightboxIndex !== null && (
        ...
        >
          ...
          <div className="flex flex-col items-center max-w-4xl max-h-[90vh]" onClick={e => e.stopPropagation()}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
